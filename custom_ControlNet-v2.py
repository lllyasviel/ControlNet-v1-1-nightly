        ...
        if len(files) > 0:
            ...
            for image_path in image_path_list:
                ...
                for image_idx in range(1,num_samples + 1):
                    ...
                    save_path = ''.join(save_path.split('_')[:-1] )+ '_00' + str(path_number) + '.' + save_path.split('.')[-1]
                    save_path = save_path.replace(root_directory, save_directory)
                    directory = os.path.sep.join(save_path.split(os.path.sep)[:-1])
                    if not os.path.isdir(directory):
                        os.makedirs(directory)
                    cv2.imwrite(save_path, return_image[image_idx])
                    path_number += 1
...
det = 'None'
a_prompt = 'best quality, extremely detailed, no human'
...
root_directory = './done_preprossed_data'  # your directory
save_directory = './save_data_8_10'  # New directory to save

make_images(root_directory, save_directory, possible_extensions)
